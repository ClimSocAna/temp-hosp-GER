...
# summarise results ------------------------------------------------------------------
M1.21 <- tidy(M1_gisd[[1]])
M1.21$age_cat <- "<30"

M1.22 <- tidy(M1_gisd[[2]])
M1.22$age_cat <- "30 to 65"

M1.23 <- tidy(M1_gisd[[3]])
M1.23$age_cat <- ">65"

M1.24 <- rbind(M1.21, M1.22, M1.23)
M1.24$group <- "All"
rm(M1.21, M1.22, M1.23); gc()

write.csv(M1.24, "Tables/coefficients_agestrat_FE.GISDint.csv")
...
mean(predict(M1_gisd[[3]], newdat),na.rm=T)

# run bootstrap -----------------------------------------------------------
boot_res <- boot(data = weekcounts_temp, statistic = boot_fun, R = 1000, newdatasets = newdat,
                 formula = formula_gisd, temp_names = temp_cats, strata_by = "age_cat",
                 strata = weekcounts_temp$age_cat, #to make sure it samples right
                 parallel = "multicore",
                 ncpus = n_cores)
...
write.csv(ci_df, "Tables/cumulativeME_FE_GISDagestrat_bin.csv")